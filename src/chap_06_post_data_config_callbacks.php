		...
		'validators' => [
				...
				'in_array' => new class () implements CallbackInterface {
					public function __invoke($item, $params): Result {
						...
						if (! $valid)
							$error [] = Messages::$messages ['in_array'];
						return new Result ( $valid, $error );
					}
				},
				...
				'length' => new class () implements CallbackInterface {
					public function __invoke($item, $params): Result {
						...
						if (isset ( $params ['min'] )) {
							$count ++;
							if (strlen ( $item ) >= $params ['min']) {
								$valid ++;
							} else {
								$error [] = sprintf ( Messages::$messages ['length_too_short'], $params ['min'] );
							}
						}
						if (isset ( $params ['max'] )) {
							$count ++;
							if (strlen ( $item ) <= $params ['max']) {
								$valid ++;
							} else {
								$error [] = sprintf ( Messages::$messages ['length_too_long'], $params ['max'] );
							}
						}
				...
				'required' => new class () implements CallbackInterface {
					public function __invoke($item, $params): Result {
						...
						$valid = boolval ( $item );
						if (! $valid)
							$error [] = Messages::$messages ['required'];
						return new Result ( $valid, $error );
					}
				} 
		],
		'filters' => [ 
				...
				'length' => new class () implements CallbackInterface {
					public function __invoke($item, $params): Result {
						...
						if ($filtered !== $item)
							$changed [] = Messages::$messages ['filter_length'];
						return new Result ( $filtered, $changed );
					}
				},
				// params: none
				'filter_float' => new class () implements CallbackInterface {
					public function __invoke($item, $params): Result {
						...
						$filtered = ( float ) $item;
						if ($filtered !== $item)
							$changed [] = Messages::$messages ['filter_float'];
						return new Result ( $filtered, $changed );
					}
				} 
		] 
